### 핵심 키워드
**특성(feature)**: 데이터를 표현하는 개별 성질  **훈련(training)**: 머신러닝 알고리즘이 데이터에서 규칙을 찾는 과정  **모델(model)**: 머신러닝 알고리즘이 구현된 객체. 종종 알고리즘 자체를 모델이라 부르기도 한다.  **정확도(accuracy)**: 정확한 답을몇 개 맞혔는지를 백분율로 나타낸 값. 사이킷런에서는 0~1 사이의 값으로 출력된다.
